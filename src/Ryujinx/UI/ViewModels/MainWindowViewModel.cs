            ...
            lock (_refreshLock)
            {
                try
                {
                    ...
                    foreach (var application in tempApplications)
                    {
                        try
                        {
                            ApplicationHelper.CreateSaveDir(application.Id, application.ControlHolder);
                        }
                        catch (Exception ex)
                        {
                    ...
                    Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        try
                        {
                            lock (_applicationsLock)
                            {
                                ...
                                sorted.Bind(out _appsObservableList).Subscribe();
                        ...
                        {
                            Logger.Error?.Print(LogClass.Application, $"Failed to refresh application list: {ex.Message}");
                        }
                    });
                }
                catch (Exception ex)
                {
                    Logger.Error?.Print(LogClass.Application, $"Exception in RefreshGrid: {ex}");
                }
            }
        }
